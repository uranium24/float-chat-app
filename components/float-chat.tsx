...
import { Send, X, User, Waves, Database, BarChart3, Globe, Download, Moon, Sun } from "lucide-react"
...
const FloatChat = () => {
  ...
  return (
    <div className={`h-screen w-full ${theme.bg} flex`}>
      {/* Chat Section */}
      <div
        className={`${isMapFullscreen ? "hidden" : isMapVisible ? "w-2/5" : "w-full"} transition-all duration-500 ease-in-out flex flex-col ${theme.chatBg} border-r ${theme.border}`}
      >
        {/* Chat Header */}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`p-3 rounded-full transition-all duration-300 ${
                  isMapVisible ? "bg-white/20 text-white" : "bg-white/10 hover:bg-white/20 text-white"
                }`}
            ...
            >
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                  message.type === "user" ? "bg-blue-600 text-white" : `${theme.botAvatar} border-2`
                }`}
              >
                {message.type === "user" ? <User className="w-5 h-5" /> : <Waves className="w-5 h-5" />}
              </div>
              <div
                className={`max-w-xs lg:max-w-md xl:max-w-lg ${message.type === "user" ? "items-end" : "items-start"}`}
              >
                <div
